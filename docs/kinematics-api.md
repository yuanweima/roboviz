...
│  │  (Pose3D →   │  │  (Workpoint →      │  │  (Interactive    │     │
│  │   IK Result) │  │   IK Result)       │  │   Ghost Control) │     │
│  └──────────────┘  └────────────────────┘  └──────────────────┘     │
└─────────────────────────────────────────────────────────────────────┘
                                  │
┌─────────────────────────────────┴───────────────────────────────────┐
...
└─────────────────────────────────────────────────────────────────────┘
                                  │
┌─────────────────────────────────┴───────────────────────────────────┐
│                      Process System (Provider)                       │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  RobotProcessProvider                                          │  │
│  │  - Manages robot + ghost state                                 │  │
│  │  - Provides useProcessRobot, useProcessGhost hooks             │  │
│  │  - Auto-renders GhostRobot in ProcessScene                     │  │
│  └────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```
...
```typescript
import { usePoseIK, type Pose3D } from '@aspect/roboviz-core';
...
Workpoint-based IK that integrates with the workpoint store.

```typescript
import { useWorkpointIK } from '@aspect/roboviz-core';

function WorkpointScene({ selectedWorkpointId }) {
  const {
    ready,
    ghostJoints,
    ghostStatus,
    workspace,
    isNearSingular,
    workpoint,        // The resolved workpoint object
    targetPose,       // Derived Pose3D from workpoint
  } = useWorkpointIK({
    robotId: 'main-robot',
    urdfContent,
    workpointId: selectedWorkpointId,
    // OR use store selection:
    // useStoreSelection: true,
    seedJoints: currentJoints,
    toolOffset,
  });

  return (
    <>
      <Robot jointAngles={currentJoints} />
      {ghostJoints && (
...
```typescript
import { useGhostPreview, type IKSolver } from '@aspect/roboviz-core';
...
```typescript
import { useRobotKinematics } from '@aspect/roboviz-core';
...
For complex scenes with robot + ghost workflow, use the Process system:
...
function WeldingControls() {
  const robot = useProcessRobot();
  const ghost = useProcessGhost();

  // Access workspace analysis from ghost
  console.log('Manipulability:', ghost.workspace?.manipulability);
  console.log('Near singular:', ghost.isNearSingular);

  // Set ghost target
  const handleWorkpointClick = (pose: Pose3D) => {
    ghost.setTarget(pose);
  };

  // Apply ghost to robot
  const handleApply = () => {
    const joints = ghost.applyToRobot();
    if (joints) {
      // Robot now at ghost position
    }
  };

  return (
    <div>
      <div>Status: {ghost.status}</div>
      <div>Manipulability: {ghost.workspace?.manipulability?.toFixed(3)}</div>
      <button onClick={handleApply} disabled={!ghost.jointAngles}>
...
function ToolChangeDemo() {
  ...
  const TOOLS = {
    welder: { position: [0, 0, 0.15], quaternion: [0, 0, 0, 1] },
...
6. **Use the Process system for complex workflows** - When you need coordinated robot + ghost + playback, use `RobotProcessProvider` instead of individual hooks.
