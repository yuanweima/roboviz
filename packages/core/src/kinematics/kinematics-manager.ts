...
class KinematicsManager {
  ...
  private async doInitialize(): Promise<void> {
    try {
      ...
      try {
        trajxModule = await import('@yuanweima/trajx-wasm') as unknown as TrajxModule & {
  ...
  getAvailableRobots(): string[] {
    ...
    // Defensive: listRobots() can throw a WASM "memory access out of bounds"
    // if the shared DH database handle has been invalidated (e.g. another
    // WasmDhDatabase was freed elsewhere). This list is informational only, so
    // never let it break solver initialization.
    try {
      return this.dhDatabase.listRobots();
    } catch (err) {
      console.warn('[KinematicsManager] getAvailableRobots() failed; continuing without the list:', err);
      return [];
    }
  }
