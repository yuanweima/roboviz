...
export function getGhostStatusFromWorkspace(
  ...
  if (workspace) {
    // Invalid configuration is always an error
    if (!workspace.isValid) {
      return 'error';
    }
    // Near singularity or low manipulability is a warning (not error)
    // If IK succeeded and we got here, the pose IS reachable
    // The workspace metrics are informational, not blocking
    if (workspace.isNearSingular || workspace.manipulability < t.manipulabilityWarning) {
      return 'warning';
    }
  }
